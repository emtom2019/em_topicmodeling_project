...
class NlpForLdaInput:
    ...
    def __init__(self, data, stop_words='default', spacy_lib='en_core_sci_md',
...
if __name__ == "__main__": # Prevents the following code from running when importing module
    ...
    lda_output = lda_model.fit_transform(nlp_data.sklearn_lda_input())
    ...
    if True:
        ...
        t0 = time()
        print("Gensim model")
        gensim_lda = gensim.models.LdaMulticore(corpus=nlp_data.gensim_lda_input(),
        ...
        print("done in %0.3fs." % (time() - t0))
        pprint(gensim_lda.show_topics(formatted=True))
        pprint(gensim_lda.show_topics(formatted=False))
